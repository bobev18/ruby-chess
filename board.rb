...
EMPTYBOARDINIT = {'a8' =>nil, 'b8' =>nil, 'c8' =>nil, 'd8' =>nil, 'e8' =>nil, 'f8' =>nil, 'g8' =>nil, 'h8' =>nil,
...
class Board
    ...
    def initialize(board_state=nil, winch=false, binch=false, debuglevel=0)
        ...
        @white_king = nil           # the position of the white king - it's useful to have quick access to that
    ...
    def initialset
        ...
        piecefy(PLAINBOARDINIT)
    end

    def piecefy(board_state)
        @board = board_state.clone
        @whites = []
        @blacks = []
        @board.keys.each do |sq|
            if @board[sq]
                if @board[sq][0] == 'w'
                    pp = Piece.new 'w', @board[sq][1], sq
                    @whites << pp#Piece.new 'w', @board[sq][1], sq
                    if @board[sq][1] == 'k' then @white_king = sq end
                else
                    ppb = Piece.new 'b', @board[sq][1], sq
                    @blacks << ppb #Piece.new 'b', @board[sq][1], sq
                    if @board[sq][1] == 'k' then @black_king = sq end
                end
            end
        end
    end
    ...
    # def hashit(board_state=nil)
    #     if not board_state
    #         @board.values.map { |value| " "  ? (not value) : value  }.join
    #     else
    #         board_state.vlaues.map { |value| " "  ? (not value) : value  }.join
    #     end
    # end
    def hashit
        @board.values.map { |value| not value ? " " : value  }.join
    ...
    def show
        ...
        8.downto(1) do |i|
            result += "|"
            97.upto(104) do |j|
                if not @board[j.chr() + i.to_s]
                    result += "  |"
                else
                    result +=  @board[j.chr() + i.to_s] + "|"
                end
            end
            result += "\n" # -- -- -- -- -- -- -- --\n"
        end
